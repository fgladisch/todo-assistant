...
import { z } from "zod";
import { getTasks, markTaskAsDoneByTitle } from "./tasks";
...
export const GET_TODOS_TOOL: StructuredTool = tool(
  async () => {
    ...
    const entries = Array.from(taskList.entries());
...
export const ADD_TODO_TOOL: StructuredTool = tool(async ({ title }) => title, {
  name: "add_todo",
...
export const MARK_TODO_AS_DONE_TOOL: StructuredTool = tool(
  async ({ title }) => {
    try {
      await markTaskAsDoneByTitle(title);
      return `The todo with the title "${title}" has been marked as done.`;
    } catch (error) {
      console.error(error);
    }
  },
